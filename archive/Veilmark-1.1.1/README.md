...
Current version: **1.1.1**. Historical local project location is intentionally omitted from the published source record.

This version fixes the clipped header subtitle, fits all eight redaction-rule rows without a vertical scrollbar, and lets the **On** header select/deselect every category. Earlier builds are preserved under `archive`; previous intermediate files are under `work`. Current validation evidence is under `validation`, and the portable ZIP is under `releases`.
