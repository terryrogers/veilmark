...
Current version: **1.2.1**. Historical local project location is intentionally omitted from the published source record.

This version retains the full configuration and window layout across restarts, including active cell edits when closing immediately. Existing 1.2.0 settings are read and upgraded automatically when saved. Earlier builds are preserved under `archive`; intermediate files are under `work`. Current validation evidence is under `validation\1.2.1`, and the portable ZIP is under `releases`.
...
The test command uses only built-in synthetic fixtures and isolated settings files; it does not read or modify your saved preferences. It writes its report and main/dialog previews under `validation` by default. The supplied 1.2.1 validation run passed 110 checks, including restoration in a separate process, an active edit at close, all layout settings, version 1.2.0 migration, settings encryption and reload, plaintext exclusion, failed/corrupt file handling, Apply/Cancel, retained lists, dialog icon and button visibility, as well as the prior detection and live-output checks. Self-tests may briefly display synthetic windows.
